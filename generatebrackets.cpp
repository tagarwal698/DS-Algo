...
void generatebrac(string str, int n, int i, int open, int close){
	if(i == 2*n) {
		cout << str << endl;	 return; 
	}

	if(open < n) {
		generatebrac(str + "(", n, i+1, open+1, close);
	}

	if(close<open){
		generatebrac(str + ")", n, i+1, open, close+1);
	}
}
